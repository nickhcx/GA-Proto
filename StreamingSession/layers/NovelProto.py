...
class IncreProto(framework.IncreFewShotREModel):
    ...
    def __init__(self, baseModel, Meta_CNN, topKEmbed, top_K):
        ...
        # checkpoint path
        pre_state_dict = torch.load(r"")['state_dict']
    ...
    def forward(self, novelSupport_set, query_set, query_label, base_label, K, hidden_size, baseN, novelN, Q, triplet_num, margin, biQ, triplet_loss_w, is_train):
        ...
        teacher_prototypes, Outl_teacher,disps_teacher = self.teacher_prototype_generator(novel_support_teacher, novelN, K, hidden_size, teacher_query)

        # novel_prototypes.shape=[B, novelN, hidden_size]  #[4, 10, 230]         # student_query.shape=[4,1,10,230]
        student_prototypes, Outl_student, novel_support_student,disps_student= self.student_prototypes_generator(novel_support_w, novelN, K, hidden_size, student_query)
    ...
    def teacher_prototype_generator(self, novel_support, novelN, K, hidden_size, teacher_query):#[4,200,1,230]
        """
        ...
        distributions = novel_support.view(-1, novelN, K, hidden_size)  # 形状变为 [4, 10, 5, 230]

        # 计算每个分布的均值向量
        mean_vectors = distributions.mean(dim=2, keepdim=True)  # 形状变为 [4, 10, 1, 230]

        mean_vectors_flat = mean_vectors.repeat(1, 1, K, 1)  # 形状变为 [4, 10, 5, 230]


        # 计算协方差矩阵
        cov_matrices = (distributions - mean_vectors_flat).transpose(3, 2) @ (distributions - mean_vectors_flat) / (
                    K - 1)+ a * torch.eye(230)[None, None, :, :].cuda()

        #det = torch.linalg.det(cov_matrices).unsqueeze(1).repeat(1, novel_support.shape[0], 1).view(
        #    teacher_query.shape[0], -1)

        # 计算协方差矩阵的逆
        inv_cov_matrices = torch.linalg.inv(cov_matrices).repeat(1, teacher_query.shape[1], 1, 1)

        # 将外部向量展平为 [4, 200, 10, 230]
        external_vectors_flattened = teacher_query.repeat(1, 1, novelN, 1).view(teacher_query.shape[0], -1, hidden_size)

        mean_vectors_flat = mean_vectors.repeat(1, 1, teacher_query.shape[1], 1).view(teacher_query.shape[0], -1,
                                                                                      hidden_size)  # 形状变为 [4, 10, 200, 230]

        # 计算外部向量到每个分布的马氏距离
        # 首先计算外部向量与均值向量的差
        diff = external_vectors_flattened - mean_vectors_flat  # 形状变为 [4, 2000, 230]

        diff_flat = diff.unsqueeze(-2)

        # 计算马氏距离
        mahalanobis_distances = (diff_flat @ inv_cov_matrices) @ diff_flat.transpose(-2, -1)  # 形状变为 [4, 200, 10]

        mahalanobis_distances_old = mahalanobis_distances.view(teacher_query.shape[0], -1)



        distributions = novel_support.view(-1, novelN, K, hidden_size).unsqueeze(1).repeat(1, teacher_query.shape[1], 1,
                                                                                           1,
                                                                                           1)  # 形状变为 [4, 200,10, 5, 230]


        external_vectors_expanded_new = teacher_query.repeat(1, 1, novelN, 1).unsqueeze(3)  # 形状变为 [4, 200, 10,1, 230]


        combined_vectors = torch.cat([distributions, external_vectors_expanded_new], dim=3)  # 形状变为 [4, 200,10, 6, 230]


        new_means = combined_vectors.mean(dim=3, keepdim=True)  # 形状变为 [4, 200,10, 1, 230]


        centered_vectors = combined_vectors - new_means  # 形状变为 [4, 200,10,6, 230]

        cov_matrices = centered_vectors.transpose(-2, -1) @ centered_vectors / (K)+ a * torch.eye(230)[None, None, :, :].cuda()

        #det_new = torch.linalg.det(cov_matrices).view(teacher_query.shape[0], -1)


        inv_cov_matrices = torch.linalg.inv(cov_matrices)

        # [4, 200,1, 1, 230]
        external_vectors_flat = teacher_query.unsqueeze(2).repeat(1, 1, novelN, 1, 1)

        #
        mahalanobis_distances = (external_vectors_flat - new_means) @ inv_cov_matrices @ (
                    external_vectors_flat - new_means).transpose(-2, -1)

        # [4, 2000]
        mahalanobis_distances_flat_new = mahalanobis_distances.reshape(teacher_query.size()[0], -1)  # 形状变为 [4, 2000]



        O = torch.sigmoid((mahalanobis_distances_old - mahalanobis_distances_flat_new) / 2)
        ...
        return teacher_prototypes, O, variances
    ...
    def student_prototypes_generator(self, novel_support, novelN, K, hidden_size, student_query):
        """
        :param novel_support: shape = [B*novelN*K, sen_len, hidden_size]
        :param novelN:
        :param K:
        ...
        distributions = novel_support_student.view(-1, novelN, K, hidden_size)  # 形状变为 [4, 10, 5, 230]
        ...
        # det = torch.linalg.det(cov_matrices).unsqueeze(1).repeat(1, novel_support.shape[0], 1).view(
        #     student_query.shape[0], -1)

        # 计算协方差矩阵的逆
        inv_cov_matrices = torch.linalg.inv(cov_matrices).repeat(1, student_query.shape[1], 1, 1)

        # 将外部向量展平为 [4, 200, 10, 230]
        external_vectors_flattened = student_query.repeat(1, 1, novelN, 1).view(student_query.shape[0], -1, hidden_size)

        mean_vectors_flat = mean_vectors.repeat(1, 1, student_query.shape[1], 1).view(student_query.shape[0], -1,
                                                                                      hidden_size)  # 形状变为 [4, 10, 200, 230]
        ...
        mahalanobis_distances_old = mahalanobis_distances.view(student_query.shape[0], -1)

        distributions = novel_support_student.view(-1, novelN, K, hidden_size).unsqueeze(1).repeat(1, student_query.shape[1], 1, 1, 1)  # 形状变为 [4, 200,10, 5, 230]

        external_vectors_expanded_new = student_query.repeat(1, 1, novelN, 1).unsqueeze(3)  # 形状变为 [4, 200, 10,1, 230]
        ...
        # [4, 200,1, 1, 230]
        external_vectors_flat = student_query.unsqueeze(2).repeat(1, 1, novelN, 1, 1)
        ...
        # [4, 2000]
        mahalanobis_distances_flat_new = mahalanobis_distances.reshape(student_query.size()[0], -1)  # 形状变为 [4, 2000]
        ...
        return novel_prototypes, O, novel_support_student, variances
    ...
    def mahalanobis_distance(vector, vector_set, cov_matrix_inv):
        """
        计算一个向量到一个向量集合的马氏距离。

        参数:
        - vector: 一个PyTorch张量，表示要计算的向量。
        - vector_set: 一个PyTorch张量，表示向量集合，其形状应为(num_vectors, dim)。
        - cov_matrix_inv: 协方差矩阵的逆，一个PyTorch张量，形状应为(dim, dim)。

        返回:
        - distances: 一个PyTorch张量，包含向量到向量集合中每个向量的马氏距离。
        """
        # 计算向量与向量集合中每个向量的差异
        differences = vector_set - vector.unsqueeze(0)  # 形状为(num_vectors, dim)

        # 计算马氏距离的平方
        squared_distances = torch.sum(differences @ cov_matrix_inv * differences, dim=1)
