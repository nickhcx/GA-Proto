...
def base_topk_selector(basemodel, sentence_encoder, init_prototypes, top_k, train_baseData_path, baserel2index_file, re_selected, is_bert, is_simQ):
    ...
    """
    if (os.path.isfile(from_project_root("BaseStage/checkpoint/topKEmbed-bert-{}.txt").format(top_k)) or \
            os.path.isfile(from_project_root("BaseStage/checkpoint/topKEmbed-glove-{}.txt").format(top_k))) and not re_selected:
        if is_bert:
            prototypes = np.loadtxt(from_project_root("BaseStage/checkpoint/topKEmbed-bert-{}.txt".format(top_k)))
        else:
            prototypes = np.loadtxt(from_project_root("BaseStage/checkpoint/topKEmbed-glove-{}.txt".format(top_k)))
    else:
        ...
        if is_bert:
            np.savetxt(from_project_root("BaseStage/checkpoint/topKEmbed-bert-{}.txt".format(top_k)), np.array(prototypes))
        else:
            np.savetxt(from_project_root("BaseStage/checkpoint/topKEmbed-glove-{}.txt".format(top_k)),
                       np.array(prototypes))
