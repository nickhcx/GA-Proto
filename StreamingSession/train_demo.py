...
from StreamingSession.layers.train_data_loader import get_loader as train_get_loader
from StreamingSession.layers.test_data_loader import get_loader as test_get_loader
from StreamingSession.layers.NovelProto import IncreProto
from StreamingSession.layers.framework import IncreFewShotREFramework
from StreamingSession.layers.PrototypeGenerator import base_topk_selector
from StreamingSession.layers.MetaCNNEncoder import MetaCNN
...
def main():
    ...
    # shared parameter
    parser.add_argument("--baseNumRels", default=20, type=int, help="number of relations")
    ...
    parser.add_argument("--base_lr", default=1e-2, type=float, help="base model learning rate")
    parser.add_argument("--base_train_iter", default=1999, help="base model train iter")
    ...
    parser.add_argument("--train_baseN", default=0, type=int, help="trainN ways in training")
    parser.add_argument("--train_novelN", default=10, type=int, help="trainBaseN ways in training")
    parser.add_argument("--train_Q", default=5, type=int, help="Q number for each relation in training")

    parser.add_argument("--test_baseN", default=0, type=int, help="trainN ways in training")
    parser.add_argument("--test_novelN", default=80, type=int, help="testBaseN ways in training")
    parser.add_argument("--test_Q", default=1, type=int, help="Q number for each relation in training")

    parser.add_argument("--K", default=10, type=int, help="K shots in testing")
    parser.add_argument("--batch_size", default=4, type=int, help="batch size training for few shot module")
    parser.add_argument("--learn_rate", default=2e-3, type=float, help="learning rate in incremental few-shot learning")
    parser.add_argument("--triplet_num", default=20, type=int, help="triplet number")
    parser.add_argument("--margin", default=5.0, type=float, help="margin distance")
    ...
    parser.add_argument("--train_iter", default=200, type=int, help="number of training iterate")
    parser.add_argument("--val_step", default=100, type=int, help="valuation per val_steps")
    parser.add_argument("--val_iter", default=100, type=int, help="valuation iterates")
    ...
    # File Path
    parser.add_argument("--dataset", default="fewrel1.0", help="base pretrained file")
    parser.add_argument("--basemodel_ckpt", default="BaseStage/checkpoint/", help="base pretrained file")
    parser.add_argument("--baserel2index_path", default="data/fewrel1.0/baserel2index.json", help="baseRel2index json path")
    parser.add_argument("--train_data_path", default="data/fewrel1.0/novel1/novel1_train_fewrel_new.json", help="base training data path")
    parser.add_argument("--test_baseData_path", default="data/fewrel1.0/novel1/novel1_test_fewrel.json", help="base testing data path")
    parser.add_argument("--test_novelData_path", default="data/fewrel1.0/novel1/novel1_test_fewrel.json", help="novel testing data path")
    ...
    if opt.embedding_type == "bert":
        is_bert = True
        # basemodel_ckpt = opt.basemodel_ckpt + "deepProto-bert-80.pth.tar"
        # fewrel1.0
        basemodel_ckpt = opt.basemodel_ckpt + "baseProto-{}-lr-{}-pl-{}-{}.pth.tar".format("bert", opt.base_lr, opt.pl_weight, opt.base_train_iter)
    ...
    """
        输出模型基本信息 
    """
    print("{}-shot IncreFew-Shot Relation Classification".format(opt.K))
...
def load_pretrain_model(base_ckpt, embedding_type, max_length, cnn_hidden_size, word_embedding_dim,
    ...
    model = BaseProto(sentence_encoder, num_rels, cnn_hidden_size, pl_weight)
